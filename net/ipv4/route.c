...
static bool rt_cache_route(struct fib_nh *nh, struct rtable *rt)
{
	...
	bool ret = true;
	//å¯¹äºŽéœ€è¦è½¬å‘çš„æ•°æ®åŒ…ï¼Œç¼“å­˜åˆ°inputä¸­
	if (rt_is_input_route(rt)) {
		p = (struct rtable **)&nh->nh_rth_input;
	} else {
	//å¯¹äºŽæœ¬æœºå‘å‡ºçš„æ•°æ®åŒ…ï¼Œç¼“å­˜åˆ°outputä¸­
		p = (struct rtable **)raw_cpu_ptr(nh->nh_pcpu_rth_output);
	}
	orig = *p;
	//cmpxchg(void *ptr, unsigned long old, unsigned long new);
	//å‡½æ•°å®Œæˆçš„åŠŸèƒ½æ˜¯ï¼šå°†oldå’ŒptræŒ‡å‘çš„å†…å®¹æ¯”è¾ƒï¼Œå¦‚æžœç›¸ç­‰ï¼Œ
	//åˆ™å°†newå†™å…¥åˆ°pträ¸­ï¼Œè¿”å›žoldï¼Œå¦‚æžœä¸ç›¸ç­‰ï¼Œåˆ™è¿”å›žptræŒ‡å‘çš„å†…å®¹ã€‚
	//å¦‚æžœåŒæ—¶æœ‰äººæ”¹åŠ¨*pï¼Œä¼šç¼“å­˜å¤±è´¥
	//è¿™æ˜¯ä¸ªåŽŸå­æ“ä½œ
...
//¶ÔÓÚ×ª·¢µÄÊý¾Ý°ü
//ÐèÒªÒ»¸öÓÐÐ§µÄoutput½Ó¿Ú
//ÒòÎªÊý¾Ý°ü»¹Î´µ½´ïÄ¿µÄµØ
			       ...
			       u8 tos, struct net_device *dev)
{
	...
	//±ÈÈçÔÚlan¿ÚÉÏÊÕµ½Êý¾Ý°üÍ¨¹ýwan¿Ú×ª·¢³öÈ¥
	//µ«ÊÇÏìÓ¦°üÔÚ¹ýwan¿ÚÊ±£¬Èç¹ûÃ»ÓÐÅäÖÃipµØÖ·»á±»¶ªÆú
...
local_input:
	...
	if (!rth)
		goto e_nobufs;
//¶ÔÓÚ±¾µØ½ÓÊÕµÄÊý¾Ý°ü
//²»ÐèÒªoutput½Ó¿Ú£¬ÒòÎªÊý¾Ý°üÒÑ¾­µ½Ä¿µÄµØ
	rth->dst.input= ip_local_deliver;
	rth->dst.output= ip_rt_bug;
#ifdef CONFIG_IP_ROUTE_CLASSID
	rth->dst.tclassid = itag;
