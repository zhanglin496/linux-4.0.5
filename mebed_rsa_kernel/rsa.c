...

{
	...
	if( mbedtls_rsa_check_pubkey(  &rsa1 ) != 0)
		printf("mbedtls_rsa_check_pubkey falied\n");
	else
		printf("mbedtls_rsa_check_pubkey success\n");

	//encrypt 
	memcpy( rsa_plaintext, RSA_PT, PT_LEN );
	if( mbedtls_rsa_pkcs1_encrypt( &rsa1, myrand, NULL, MBEDTLS_RSA_PUBLIC, PT_LEN,
                           rsa_plaintext, rsa_ciphertext ) != 0 )
		printf("mbedtls_rsa_pkcs1_encrypt failed\n");
	else
		printf("mbedtls_rsa_pkcs1_encrypt success\n");
	fp = fopen("en.c", "w");
	fwrite(rsa_ciphertext, KEY_LEN, 1, fp);
	...
	if( mbedtls_rsa_pkcs1_decrypt( &rsa1, myrand, NULL, MBEDTLS_RSA_PRIVATE, &len,
		...
		printf("mbedtls_rsa_pkcs1_decrypt failed\n");
	else
		printf("mbedtls_rsa_pkcs1_decrypt success, len=%d\n", len);
	fp = fopen("en.d", "w");
	fwrite(rsa_decrypted, len, 1, fp);
