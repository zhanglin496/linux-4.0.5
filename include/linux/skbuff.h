 ...
 */
//之所以要同时判断引用计数，
//比如clone一个skb后，原始的skb和clone 后的nskb 的cloned 标志都为1
//如果后来nskb被释放，但是原始的skb 的cloned 标志仍为1
//所以要加上引用计数的判断防止误判
//这样就不需要重新分配skb 线性数据区，以提高效率
...
static inline int pskb_may_pull(struct sk_buff *skb, unsigned int len)
{
	...
	//和skb_make_writable的区别是，skb_make_writable保证是可写的
	//但是pskb_may_pull不保证可写，只保证skb 线性区中有指定的len 字节数据
	//同时保证指定的len字节位于线性区中
