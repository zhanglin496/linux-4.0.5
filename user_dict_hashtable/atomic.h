#ifndef _LIBC_ATOMIC_H

#define _LIBC_ATOMIC_H
...
static inline int
atomic_cas (atomic_t * v, int oldval, int newval)
{
	return __sync_bool_compare_and_swap (&v->counter, oldval, newval);
}

static inline int
atomic_xchg(atomic_t * v, int newval)
{
	return __sync_lock_test_and_set(&v->counter, newval);
}
