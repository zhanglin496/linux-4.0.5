...
{
	int err, i, fd, v = TPACKET_V1;
	struct sockaddr_ll ll;
	unsigned int blocksiz = 1 << 16; //	64KB
	unsigned int framesiz = 1 << 11; //2KB
...
static void display(struct tpacket_hdr *ppd)
{
	...
	if (eth->h_proto == htons(ETH_P_IP)) {
		...
		printf("smac "MAC2_FMT", snapelen %d, len %d, %s -> %s\n", MAC_ARG(eth->h_source), ppd->tp_snaplen, ppd->tp_len, sbuff, dbuff);
	}
//	printf("rxhash: 0x%x\n", ppd->hv1.tp_rxhash);
}

static void walk_block2(struct block_desc *pbd, const int block_num)
{
//	int num_pkts = pbd->h1.num_pkts, i;
	...
	for (i = 0; i < ring.req.tp_block_size/ring.req.tp_frame_size; ++i) {
		bytes += ppd->tp_snaplen;
		ppd = (void *)pbd + i*ring.req.tp_frame_size;
		printf("display, block_num=%d, i=%d, ppd=%p,status=%u,sec=%u\n", block_num, i, ppd, ppd->tp_status, ppd->tp_sec);
