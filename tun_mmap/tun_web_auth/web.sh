...
#		iptables -t mangle -A $AUTH_CHAIN -p tcp --dport 80 -j CONNMARK --restore-mark
		iptables -t mangle -A $AUTH_CHAIN -p tcp --dport 80 -m mark --mark 0x33 -j RETURN
		...
		iptables -t mangle -A $AUTH_CHAIN -p tcp --dport 80 -j MARK --set-mark 0x33
#		iptables -t mangle -A $AUTH_CHAIN -p tcp --dport 80 -m mark --mark 0x33 -j CONNMARK --save-mark
		iptables -t mangle -A $AUTH_CHAIN -p tcp --dport 80 -m mark --mark 0x33 -j RETURN
		...
		ip ru add fwmark 0x33 lookup 110
	;;
	del)
		killall -9 auth-box
		...
		ip ru del fwmark 0x33 lookup 110
	;;
esac
